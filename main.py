...
def calculate_possession(ball_positions, team1_color, team2_color):
    team1_possession = 0
    team2_possession = 0
    for ball_position in ball_positions:
        nearest_team = identify_nearest_team(ball_position, team1_color, team2_color)
        if nearest_team == "team1":
            team1_possession += 1
        elif nearest_team == "team2":
            team2_possession += 1
    total_possession = team1_possession + team2_possession
    if total_possession > 0:
        team1_percentage = (team1_possession / total_possession) * 100
        team2_percentage = (team2_possession / total_possession) * 100
    else:
        team1_percentage = 50
        team2_percentage = 50
    return team1_percentage, team2_percentage

def identify_nearest_team(ball_position, team1_color, team2_color):
    # Implement logic to identify nearest team based on ball position
    # This function should return "team1" or "team2" based on which team's color is nearest to the ball position
    pass

def plot_possession_chart(team1_percentage, team2_percentage, team1_name, team2_name):
    fig, ax = plt.subplots()
    ax.barh([team1_name, team2_name], [team1_percentage, team2_percentage], color=['#1E2530', '#FBFCFA'])
    ax.set_xlim(0, 100)
    ax.set_xlabel('Possession (%)')
    ax.set_title('Live Ball Possession')
    st.pyplot(fig)
...
def main():
    ...
    stframe = st.empty()
    possession_chart = st.empty()
    cap = cv2.VideoCapture(tempf.name)
    status = False

    # Initialize ball possession data
    ball_positions = []
    ...
    if start_detection and not stop_detection:
        ...
        while cap.isOpened():
            ...
            # Add the code to detect ball position and add to ball_positions list

            team1_percentage, team2_percentage = calculate_possession(ball_positions, st.session_state[f"{team1_name} P color"], st.session_state[f"{team2_name} P color"])
            plot_possession_chart(team1_percentage, team2_percentage, team1_name, team2_name)
